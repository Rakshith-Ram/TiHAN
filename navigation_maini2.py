###########################################  LIDAR NAVIGATION CODE FOR MAINI  ###########################################
...
def calc_steer_output(required_bearing, Ld_steer):
    ...
    steering_output = 34.5 * np.arctan( -1 * 2 * 3.5 * np.sin(np.pi * bearing_diff / 180) / Ld_steer )
...
def calc_velocity():
    ...
    velocity_steer = 40 + 60 * (30 - abs(steering_output)) / 30                          # linear

    velocity_brake = max(0, (100 - 70 * math.exp(-0.23 * (obstacle_distance - 2.5))))
...
def find_nearest_waypoint_index(waypoints):
    ...
    for i in range(len(waypoints) - 1):
        ...
        if distance < min_distance and distance < distance_threshold: 
            ...
            next_index = i
    print("The next nearest wapoint index is : ", next_index)
    return(next_index)



waypoints = extract_waypoints_from_file("/home/infy/python_codes_rakshith/waypoints_testbed")
...
pub_v = rospy.Publisher('/mpc/velocity_value', Float64, queue_size=10)
velocity_v = Float64()
...
def publish_velocity(velocity_value_data):
    ...
    velocity_v.data = velocity_value_data 
    pub_v.publish(velocity_v)



pub_cart_control(1)     # Function call
...
while True:
    ...
    if (wp < len(waypoints)-1):
        ...
        temp_velocity = vehicle_velocity
        if (vehicle_velocity < velocity_value):
            temp_velocity = temp_velocity + 20
            publish_velocity(temp_velocity)
            print("Velocity : ", temp_velocity)
